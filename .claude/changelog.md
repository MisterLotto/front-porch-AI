...
## 2026-06-07 (Comprehensive Rawhide CI fix: 6 failures + 2 intermittent test flakes)
- Branch: `fix/rawhide-ci-fixes` (off `upstream/Rawhide`). Single coherent pass after two earlier targeted PRs were closed — neither greened CI alone, and diagnosis revealed intermittent flakes that the "one small fix at a time" approach kept hiding. Full suite now 890 tests green; realism file verified 15/15 runs.
- **Stale tests (test-only).** `avatar_repository_test.dart` schema assertion 29→32 (matches `AppDatabase.schemaVersion`). `chat_message_test.dart` ×3 realigned to the `ChatMessage` ctor's out-of-range `swipeIndex` clamp-to-0 (the throwsRangeError case became clamp-to-0; two metadata cases now pass a `swipes` list long enough to keep swipeIndex valid instead of being silently clamped).
- **Bond-doubling regression (PROD).** `startNewChat` 1:1 seeding ran the legacy ±150→±300 migration (`_migrate*`, ×2 for |v|≤150) on the card's `shortTermBond`/`longTermBond`. Card seeds are already on the ±300 scale (`character_card.dart:31-32`), so it double-counted (55→110); `trustLevel` in the same block was never migrated, confirming the bug; nonlinear (200 stays 200). Seed now plain `.clamp()`; migration kept only on legacy session loads (`_loadLastSession`/`loadSession`). Gated on `_activeGroup == null` → group parity preserved. User-facing note added.
- **One-shot prompt-marker (test-only).** One-shot path fires + applies deltas fine; test asserted markers from the wrong prompt. Now asserts `relationship_delta` && `trust_delta` (actually in `_evaluateOneShotCall`'s prompt).
- **Dispose-crash flake (PROD).** `ChatService._loadActiveObjectives` (and others) await DB work then `notifyListeners()`; if the service was disposed during the await (chat closed mid-load, or test tearDown) it threw "A ChatService was used after being disposed". Added a `_disposed` flag + central overrides of `notifyListeners` (no-op after dispose) and `dispose` (sets flag) — guards all ~120 call sites without per-site checks. Also a real-app stability fix. Verified gone over 12 runs.
- **Chaos-hang flake (test-only).** Diagnosed from logs: `[ChanceTime] Auto-trigger!` immediately preceded every 30s timeout. When Chaos Mode auto-fires (random ~5% roll), `sendMessage` awaits `_chanceTimeCompleter`, which is only completed by the UI wheel (`applyChanceTimeResult`) — never in a headless test → hang. NOT a prod bug (UI resolves it live). Fix: the small-group realism test enabled `chaosModeEnabled` but never asserted chaos, so disabled it there (both GroupsCompanion + GroupChat). Verified 15/15 runs green.
- Files: `lib/services/chat_service.dart`, `test/services/avatar_repository_test.dart`, `test/services/chat_message_test.dart`, `test/services/chat_service_realism_engine_test.dart`, `docs/Rawhide.md`, `.claude/changelog.md`
- Hygiene: New private methods: 0 (added 1 field `_disposed` + 2 `@override`s, no new privates). Methods deleted: 0 (`_migrate*` helpers still used by session loads — not dead). `flutter analyze` on changed files clean (only 8 pre-existing `unintended_html_in_doc_comment` infos in chat_service.dart, none on changed lines). Full suite 890 green. Lesson recorded: do not trust "passes in isolation" or a single green run for the realism file — flakes (dispose race, chaos wheel) only surface intermittently; run the full file many times.
