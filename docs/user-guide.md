...
**Security:** web access has its own login, which you create in the browser the first time you connect. Sessions are per-device, two-factor authentication is optional, and the desktop side is your recovery key: Settings → Advanced → Web Server can **sign out all devices** or **reset the web login** entirely if you ever get locked out. (Resetting only clears the web username, password and 2FA — your characters, chats and settings aren't touched.)
