...
| Native spell check answers on all 3 desktops | `spell_check` | real `front_porch_ai/spell_check` channel, no fake; exact UTF-16 span bounds (emoji case is the point); widget-level pipeline check via real keystrokes; Linux-scoped false-positive rules; asserts the bundled dictionary shipped. Needs NO packages installed — CI installs none on purpose, so it exercises the out-of-the-box path |
