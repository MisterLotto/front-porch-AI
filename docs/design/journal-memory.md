...
4. **Polish (shipped):** tool-calling transport + review-first mode.
   - *Tools:* `LLMService.generateWithTools` (default null = unsupported; local KoboldCpp stays null by design — the XML floor). `OpenRouterService` implements it non-streaming over the shared payload builder; `kJournalTools` schemas + `parseJournalToolCalls` in journal_ops normalize to the same `JournalOp` list (one applier, two front doors, §4.3). The pass probes tools once per backend identity per run (`_runExchange`), salvages XML tags out of a text-only reply, and remembers XML-only backends — at most one extra round trip per backend.
