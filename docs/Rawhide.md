...
- 🔁 **Update a character you've shared instead of re-posting it** — the **Mine** tab now has an **Update** button on each character you've shared (works for approved *and* pending ones). It opens the **full character editor** you already know — Details, Dialogue, Lorebook, Worlds, Realism, everything — and **Next** saves your edits to your library *and* re-publishes the shared card **in place**, keeping the same page, downloads, votes, and links. The detail panel shows a version pill (**v2**, **v3**…) once a card has been updated, and re-sharing the same character no longer creates a duplicate. (Solo characters now; group cards are coming next.)
