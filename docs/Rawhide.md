...
- 🖥️ **Older PCs are supported now (no AVX2? no problem)** — the AI engine's standard download needs a newer CPU feature (AVX2) and would crash on launch on older or budget machines, leaving those users stuck. Front Porch AI now detects a non-AVX2 CPU on Windows and Linux and automatically grabs KoboldCpp's compatible "old PC" build instead — so it just works, no manual downloads or guesswork.
