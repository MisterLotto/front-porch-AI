...
- 🔁 **Update a character you've shared instead of re-posting it** — the **Mine** tab now has an **Update** button on each character you've shared (works for approved *and* pending ones). It re-publishes your latest version from your library **in place** — keeping the same page, its downloads, votes, and links — and the detail panel shows a version pill (**v2**, **v3**…) once a card has been updated. Re-sharing the same character no longer creates a duplicate. (Solo characters now; group cards are coming next.)
