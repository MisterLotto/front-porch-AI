...
- 🔁 **Update a character you've shared instead of re-posting it** — the **Mine** tab now has an **Update** button on each character you've shared (works for approved *and* pending ones). It re-publishes **in place** — keeping the same page, its downloads, votes, and links — and you can **edit the card's content by hand** (persona, personality, scenario, first message, example dialogue) right in the update flow, or just push your latest library version. The detail panel shows a version pill (**v2**, **v3**…) once a card has been updated, and re-sharing the same character no longer creates a duplicate. Your library copy is never changed by a Stoop edit. (Solo characters now; group cards are coming next.)
