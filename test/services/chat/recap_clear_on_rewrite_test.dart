...
// a timeline rewrite. Cards purge by receipt; the recap is free-form, so the
// honest fix is CLEAR on regen/delete/swipe-invalidate — not leave a stale
// paragraph until the next journal interval.
//
// Guard proven to fail before passing: with the _summary = '' clear removed
// from _invalidateJournalFrom, deleteMessage leaves the planted recap text.
...
void main() {
  ...
  test('deleteMessage clears a planted recap (honest empty until next pass)',
      () async {
  ...
  test('regenerateLastMessage clears recap before the new reply is built',
      () async {
    final card = CharacterCard(
      name: 'Mara',
      description: 'Exists only for the recap-clear test.',
    ...
    chat.setSummary(
      'We never left the living room. Leo is still in the hall.',
    );
    expect(chat.summary, isNotEmpty);
    ...
    expect(
      ...
      isEmpty,
      reason:
          'regen must clear the recap so the next prompt does not re-assert '
          'the discarded scene as "earlier in this story"',
    );
  });
}
